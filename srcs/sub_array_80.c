...
void    first_shift(char product[5000][1000], t_float *fnum, unsigned int power)
{
  unsigned int offset;
...
void    sum_power(char product[5000][1000], t_float *fnum)
{
  unsigned int power;
  unsigned int offset;
  unsigned int carry;
  ...
  while (power > 0)
  {
    ...
    {
      offset = power + 16382 - ft_strlen(product[power + 16382]);
      i = ft_strlen(product[power + 16382]) - 1;
      carry = 0;
      while (product[power + 16382][i])
      {
        fnum->big_str[offset + i] += product[power + 16382][i] + carry - '0';
        carry = 0;
...
void    aggregate(char product[5000][1000])
{
    int i;
    int j;
    unsigned int tmp;
