...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   print_g_str_helper.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hlo <marvin@42.fr>                         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/19 14:34:54 by hlo               #+#    #+#             */
/*   Updated: 2019/10/19 14:38:40 by hlo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	remove_finalz_help(char *final, int exp, t_float *fnum, t_block *blksk)
{
	int tmp;
...
void	remove_finalz(char *final, t_block *blksk, int exp, t_float *fnum)
{
	...
	count_del = 0;
	tmp = 0;
	remove_finalz_help(final, exp, fnum, blksk);
	if (!((blksk->type == 'f') || (blksk->type == 'F')))
	{
		if (blksk->type == 'e')
			while (final[exp] != 'e')
		...
		count_del = exp - tmp;
		if (count_del != 0)
			ft_memmove(&final[tmp + 1],
					&final[exp + 1], ft_strlen(&final[exp + 1]));
	}
}
...
void	dup_g(t_float *fnum, t_float *fnume, t_block *blksk, t_block *blkse)
{
